...
public class MyAppDeleteAll {

	public MyAppDeleteAll() throws IOException {

		String[] lines = AppGUI.getNote().getText().split("\\n");

		Writer w = new BufferedWriter(new OutputStreamWriter(
				new FileOutputStream("usuwaj_konta.cmd"), "UTF-8"));
		...
		for (int j = 0; j < lines.length; j++) {
			...
			if (lines[j] == null || lines[j].equals("")) {
				...
				w.write("call czysc.cmd WSZYSTKO " + shortName);
